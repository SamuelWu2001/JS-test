...
function five(daytime){
    time = []
    var totalNum = 0
    for(var i=0;i<employeeType.length;i++){
        if (stringToDate(day+daytime).getTime()>=stringToDate(day+employeeType[i].work_begin).getTime() && stringToDate(day+daytime).getTime()<=stringToDate(day+employeeType[i].work_end).getTime()){
            time.push(1)
        }
        else{
            time.push(0)
    ...
    for(var i=0;i<employees.length;i++){
        totalNum += time[employees[i].type-1]
    }
